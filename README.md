...
### env
Выполнять только если ваша база данных postgresql, иначе пропустить это шаг.

Создать файл .env со следующими строками для базы данных:
- DATABASE_NAME = <Имя для подключения к базе данных> Стандартно <postgres>
- DATABASE_USERNAME = <Имя пользователя для подключения к базе данных> <postgres>
- DATABASE_PASSWORD = <Пароль для подключения к базе данных> <postgres>
- DATABASE_HOST = <Адрес для подключения к базе данных> <localhost>
- DATABASE_PORT = <Порт для подключения к базе данных> <5432>
...
Создать пользователя-бота и по [адресу]([http://127.0.0.1:8000/admin/](http://127.0.0.1:8000/admin/authtoken/tokenproxy/)) нужно создать токен для бота.
